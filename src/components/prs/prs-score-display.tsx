...
const fetchPrsScores = async (userId: string): Promise<PrsScore[]> => {
  try {
    ...
    return items.map((item: any) => ({
      ...
      score: item.score || 0,
      percentile: Math.round((item.score || 0) * 100),
      riskLevel: (item.score || 0) > 0.7 ? 'high' : (item.score || 0) > 0.4 ? 'moderate' : 'low',
      description: `${(item.score || 0) > 0.7 ? 'Higher' : (item.score || 0) > 0.4 ? 'Moderate' : 'Lower'} than average genetic risk for ${item.disease_type ? item.disease_type.replace(/_/g, ' ') : 'this condition'}`
    }));
  } catch (error) {
